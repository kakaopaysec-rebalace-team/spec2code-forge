...
이제 **완전 무료 LLM**을 사용한 고급 전략 분석이 가능합니다!
...
# 2. 무료 모델 다운로드 (Llama 3.1 8B - 추천)
ollama pull llama3.1:8b

# 3. Ollama 서비스 시작
ollama serve

# 4. 시스템에서 자동으로 무료 LLM 사용 시작!
```

#### 지원 기능
- **📊 맞춤 전략 학습**: URL, 파일, 메모 분석을 통한 개인화된 투자 전략 생성
- **🧠 지능형 포트폴리오 분석**: AI가 투자 성향, 목표, 선호도를 파악
- **💡 실시간 투자 조언**: 시장 상황과 사용자 데이터를 종합한 전문가급 분석
- **🔄 자동 Fallback**: Ollama 불가시 Claude API → Database AI → 규칙 기반 순으로 자동 전환

#### 무료 LLM vs 유료 API 비교
| 기능 | 무료 LLM (Ollama) | Claude API |
|------|------------------|------------|
| 비용 | 완전 무료 | 사용량당 과금 |
| 프라이버시 | 완전 로컬 처리 | 외부 전송 |
| 속도 | 로컬 처리 (빠름) | 네트워크 의존 |
| 품질 | 우수 (Llama 3.1) | 최고급 |
| 설정 | 한번만 설치 | API 키 필요 |
